    ...
    <ul className="grid group/list">
      {posts.slice(0, limit ?? posts.length).map(({ meta, slug }, i) => (
        <Link key={i} href={`/blog/${slug}`}>
          <li
            key={meta.title}
            className="flex justify-between items-center gap-3 group-hover/list:opacity-75 group/item hover:bg-zinc-100/50 dark:hover:bg-zinc-800/50 hover:!opacity-100 transition-all duration-200  p-2  rounded-md cursor-pointer -mx-2"
          >
