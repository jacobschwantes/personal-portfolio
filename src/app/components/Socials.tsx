...
const platforms = [
  {
    name: "Email",
    href: "mailto:jacobschwantes@outlook.com",
    icon: (props: { className: string }) => (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className={clsx(props.className, "h-[18px] w-[18px]")}
      >
  ...
  <div className="flex gap-3 items-center">
    {platforms.map((platform) => (
      <Link
        ...
        href={platform.href}
      >
        <platform.icon className="w-4 h-4 text-zinc-400 group-hover:text-yellow-400 transition-colors duration-300" />
      </Link>
    ))}
  </div>
);
